# Updating a v0.2 Vault to v0.3

Version 0.3 adds consequence-based automation without changing personal notes or `System/config.json`.
...
The watcher now runs `refresh` by default. Set `on_change` to `index` in `System/automation.json` for the previous lightweight behavior.
