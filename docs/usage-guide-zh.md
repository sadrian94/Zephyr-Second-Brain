...
Zephyr 的原則很簡單：捕捉要輕，承諾要清。日常操作以你和 AI agent 對話為主；CLI 是在你明確決定後，由代理或你自己執行的可稽核交易層。
...
```text
記下這個想法：我想比較本機語言模型是否適合私密研究。
```

```text
蒸餾 Capture/文章剪藏.md；保留來源，給我看草稿。
```
...
```text
我批准「本地模型研究」成為專案；先預演，再套用。
```

一次直接請求只授權一份新的原始 Capture 或伴隨草稿。代理可以 refresh 生成資料並說明提案，卻不能把草稿自行變成 Active 專案、Brain 筆記、Archive 記錄，或改寫你的文字；那些都需要新的明確批准。

## 日常流程

1. 請代理把想法、剪藏或日誌放進 `Capture/`。原始 Capture 不需要 YAML。
2. 代理執行安全更新：
...
直接對話不需要開啟長期草稿自動化：你的當次請求已授權代理在 `Capture/` 建立一份不撞名的 `-- draft.md` 伴隨草稿；它必須保留來源，不能啟用、升格、封存、刪除或改寫內容。

排程式代理草稿自動化則預設關閉。明確啟用後，外部代理平台可以在沒有即時對話時建立同類草稿。
...
直接說「我批准這篇升格到 Brain；先預演，再套用」。代理應執行：
...
先明確批准把專案狀態改成 `completed` 或 `stopped`，再請代理預演並封存：
