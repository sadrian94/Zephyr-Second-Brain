...
def classify_and_tag(file_path, content, config):
    ...
    if not api_key or "YOUR" in api_key or api_key.startswith("<"):
        print("Skipping AI classification: No valid 'ai_api_key' configured in System/config.json")
        return None
    ...
    for fname in os.listdir(CAPTURE_DIR):
        ...
        for idx, line in enumerate(lines):
            ...
            if match:
                ...
                if "[[" not in idea_text or "]]" not in idea_text:
                    unprocessed.append({
                        "source_log": rel_path,
                        "line_number": idx + 1,
                        "text": idea_text
...
def compile_index():
    ...
    def scan_dir(dpath):
        ...
        for fname in os.listdir(dpath):
            ...
            summary = clean_body[:150].strip().replace("\n", " ") + ("..." if len(clean_body) > 150 else "")
